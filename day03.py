...
def check(lines: List[str], from_col : int, to_col : int, row: int, condition: Callable) -> Optional[Tuple[int, int]]:
    max_row = len(lines) - 1
    i_from, i_to = max(0, from_col - 1), min(len(lines[0]) - 1, to_col + 1)
    for i in range(i_from, i_to + 1):
        if row > 0:
            if condition(lines[row - 1][i]):
                return (row-1, i)
        if row < max_row:
            if condition(lines[row + 1][i]):
                 return (row+1, i)
        if (i == i_from and from_col !=0) or (i == i_to and to_col != len(lines[0])-1):
            if condition(lines[row][i]):
                return (row, i)
    return None
    ...
    for row, line in enumerate(lines):
        ...
        for col, char in enumerate(line):
            ...
            if len(current) > 0 and (not char.isdigit() or col == len(line) - 1):
                value = int(current)                
                if check(lines, start_col, start_col + len(current) - 1, row, lambda x: x != '.'):
                    total += value
                current = ''
    return total
...
def part2(lines: List[str]) -> int:
    ...
    for row, line in enumerate(lines):
        ...
        for col, char in enumerate(line):
            ...
            if len(current) > 0 and (not char.isdigit() or col == len(line) - 1):
                value = int(current)
                check_result = check(lines, start_col, start_col + len(current) - 1, row, lambda x: x == '*')
                if check_result:
